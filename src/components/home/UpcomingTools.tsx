    ...
    <section id="upcoming" className="container mx-auto px-6 mb-32">
      <div className="flex flex-col md:flex-row items-end justify-between mb-12 gap-4">
        <div className="max-w-xl">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-gray-900 dark:text-white mb-4">Upcoming Tools</h2>
          <p className="text-sm md:text-base text-gray-500 dark:text-gray-400">Sneak peek at what we're building next. Vote for your favorites to speed up development.</p>
        </div>
      </div>

      <div
        ref={sectionRef as React.RefObject<HTMLDivElement>}
        className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 on-scroll"
      >
        {upcomingTools.map((tool, i) => (
          <div
            key={tool.id}
            className={`group relative p-6 rounded-[1.5rem] bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 overflow-hidden anim-fade-up h-full flex flex-col`}
        ...
        <div className="p-6 rounded-[1.5rem] border-2 border-dashed border-gray-200 dark:border-white/10 flex flex-col items-center justify-center text-center group hover:border-brand-500/50 transition-colors anim-fade-up anim-delay-3 h-full">
          <div className="w-12 h-12 rounded-xl bg-brand-500/10 text-brand-500 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
